...
export function GTMTab() {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.url) { setError(data.error ?? "Nie udało się wygenerować URL autoryzacji."); return; }
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      if (!res.ok) { const data = await res.json() as { error?: string }; setError(data.error ?? "Nie udało się rozłączyć konta."); return; }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 gap-3" style={{ color: "var(--text-muted)" }}>
        ...
        <span className="text-sm">Sprawdzanie statusu GTM...</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {polling && (
        <div className="px-4 py-3 text-sm flex items-center gap-3" style={{ background: "var(--accent-subtle)", border: "1px solid rgba(114,13,214,0.2)", color: "var(--text-secondary)", borderRadius: R }}>
          ...
          Oczekiwanie na autoryzację w nowym oknie...
        </div>
      )}
      ...
      {status?.connected ? (
        <div className="p-5 flex flex-col gap-4" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: R }}>
          ...
          <div style={{ background: "var(--background)", border: "1px solid var(--border)", borderRadius: R }}>
            {status.email && <InfoRow label="Konto Google" value={status.email} mono />}
            {status.connected_at && (
              <div style={{ borderTop: status.email ? "1px solid var(--border)" : undefined }}>
                <InfoRow label="Połączono" value={new Date(status.connected_at).toLocaleString("pl-PL", { dateStyle: "medium", timeStyle: "short" })} />
              </div>
            )}
          </div>

          <p className="text-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
            Dostęp obejmuje odczyt kontenerów GTM i edycję tagów. Autoryzacja jest bezpiecznie przechowywana w Google Secret Manager.
          </p>
        </div>
      ) : (
        <div className="p-10 flex flex-col items-center gap-4 text-center" style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: R }}>
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Brak połączenia z GTM</p>
            <p className="text-xs mt-1 max-w-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
              Połącz konto Google, aby zarządzać tagami, triggerami i zmiennymi w GTM.
            </p>
          </div>
          <button
          ...
          >
            ...
            {polling ? "Oczekiwanie na autoryzację..." : "Połącz z GTM"}
          </button>
        </div>
      )}
    </div>
