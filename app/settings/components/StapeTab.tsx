...
interface StapeStatus {
  connected: boolean;
}

const R = "4px";

export function StapeTab() {
  const [status, setStatus] = useState<StapeStatus | null>(null);
  ...
  const fetchStatus = async (): Promise<StapeStatus | null> => {
    try {
      const res = await fetch("/api/stape/auth/status");
      if (res.ok) {
        const data = await res.json() as StapeStatus;
        setStatus(data);
        return data;
      }
    } catch {
  ...
  const handleConnect = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/stape/auth/start");
      const data = await res.json() as { url?: string; alreadyAuthorized?: boolean; error?: string };
  ...
  const handleDisconnect = async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/stape/auth/disconnect", { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json() as { error?: string };
        setError(data.error ?? "Nie udało się rozłączyć konta Stape.");
        return;
      }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16 gap-3" style={{ color: "var(--text-muted)" }}>
        ...
        <span className="text-sm">Sprawdzanie statusu Stape...</span>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      {polling && (
        <div className="px-4 py-3 text-sm flex items-center gap-3" style={{ background: "var(--accent-subtle)", border: "1px solid rgba(114,13,214,0.2)", color: "var(--text-secondary)", borderRadius: R }}>
          ...
          Oczekiwanie na autoryzację Stape w nowym oknie...
        </div>
      )}
      ...
      {status?.connected ? (
        <div className="p-5 flex flex-col gap-4" style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: R }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Stape MCP Server</p>
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="w-1.5 h-1.5 rounded-full" style={{ background: "var(--success)" }} />
                  <span className="text-xs font-medium" style={{ color: "var(--success)" }}>Połączono · OAuth 2.0</span>
                </div>
              </div>
            </div>
            ...
            {!confirmDisconnect ? (
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs" style={{ color: "var(--text-secondary)" }}>Potwierdzasz?</span>
                <button onClick={handleDisconnect} disabled={actionLoading} className="text-xs px-3 py-1.5 font-semibold" style={{ background: "var(--error)", color: "#fff", borderRadius: R, opacity: actionLoading ? 0.5 : 1 }}>
                  {actionLoading ? "Rozłączanie..." : "Tak, rozłącz"}
                </button>
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
            Połączono ze Stape MCP Server. Asystent AI może teraz zarządzać tagami, triggerami i zmiennymi GTM przez Stape. Autoryzacja jest bezpiecznie przechowywana w Google Secret Manager.
          </p>
        </div>
      ) : (
        <div className="p-10 flex flex-col items-center gap-4 text-center" style={{ background: "var(--surface)", border: "1px dashed var(--border)", borderRadius: R }}>
          ...
          <div>
            <p className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>Brak połączenia ze Stape MCP</p>
            <p className="text-xs mt-1 max-w-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
              Połącz konto Stape, aby asystent AI mógł wykonywać operacje GTM przez serwer Stape MCP.
            </p>
          </div>
          <button
            ...
            style={{ background: "var(--accent)", color: "#000000", borderRadius: R, opacity: (actionLoading || polling) ? 0.6 : 1 }}
          ...
          >
            ...
            {polling ? "Oczekiwanie na autoryzację..." : "Połącz ze Stape"}
          </button>
        </div>
      )}
    </div>
