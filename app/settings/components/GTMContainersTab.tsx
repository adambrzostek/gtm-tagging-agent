...
export function GTMContainersTab() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setError("Nie udało się załadować listy kontenerów.");
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-48">
          ...
          <input type="text" placeholder="Szukaj kontenera..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-8 pr-3 py-2 text-sm outline-none" style={{ background: "var(--surface)", border: "1px solid var(--border)", color: "var(--text-primary)", borderRadius: R }} />
        </div>
        ...
        <select value={accountFilter} onChange={(e) => setAccountFilter(e.target.value)} className="py-2 pl-3 pr-8 text-sm outline-none appearance-none cursor-pointer" style={{ background: "var(--surface)", border: "1px solid var(--border)", color: accountFilter === "all" ? "var(--text-secondary)" : "var(--text-primary)", borderRadius: R }}>
          <option value="all">Wszystkie konta GTM</option>
      ...
      {loading && !error && (
        <div className="flex items-center justify-center py-16 gap-3" style={{ color: "var(--text-muted)" }}>
          ...
          <span className="text-sm">Ładowanie kontenerów...</span>
        </div>
      )}
        ...
        <>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <Th>Kontener</Th><Th>ID</Th><Th>Domena</Th><Th>Kraj</Th><Th>Konto GTM</Th><Th>Typ</Th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={7} className="py-10 text-center text-sm" style={{ color: "var(--text-muted)" }}>Brak wyników</td></tr>
                ) : (
                  filtered.map((c, i) => {
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              Wybrano {selectedInFiltered} z {filtered.length} kontenerów
            </p>
            <div className="flex items-center gap-3">
              {saveError && <span className="text-xs" style={{ color: "var(--error)" }}>{saveError}</span>}
              {savedOk && <span className="text-xs flex items-center gap-1" style={{ color: "var(--success)" }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12" /></svg>Zapisano</span>}
              <button onClick={handleSave} disabled={!isDirty || saveLoading} className="flex items-center gap-2 text-sm px-4 py-2 font-semibold" style={{ background: isDirty ? "var(--accent)" : "var(--surface-elevated)", color: isDirty ? "#000000" : "var(--text-muted)", borderRadius: R, opacity: saveLoading ? 0.6 : 1, cursor: isDirty ? "pointer" : "default" }} onMouseEnter={(e) => { if (isDirty && !saveLoading) e.currentTarget.style.background = "var(--accent-hover)"; }} onMouseLeave={(e) => { e.currentTarget.style.background = isDirty ? "var(--accent)" : "var(--surface-elevated)"; }}>
          ...
          <p className="text-xs" style={{ color: "var(--text-muted)", lineHeight: "1.6" }}>
            Wybierz kontenery dostępne dla agenta. Brak wyboru = agent nie ma dostępu do GTM.
          </p>
        </>
      )}

      {!loaded && !loading && !error && (
        <EmptyState icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="23 4 23 10 17 10" /><polyline points="1 20 1 14 7 14" /><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" /></svg>} title="Kontenery nie załadowane" description="Kliknij Odśwież, aby załadować kontenery GTM." />
      )}

      {loaded && !loading && !error && containers.length === 0 && (
        <EmptyState icon={<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M9 21V9" /></svg>} title="Brak kontenerów GTM" description="Konto GTM nie ma dostępu do żadnych kontenerów." />
      )}
    </div>
  );
}
